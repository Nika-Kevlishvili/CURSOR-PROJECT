...
| **5. Risk coverage (cross_dep)** | 10 | **10**: TC explicitly names and tests ≥1 item from `what_could_break` or `integration_points`. **6**: TC addresses a cross-dep risk implicitly. **3**: TC is in unrelated domain (stated clearly why cross-dep N/A); **or** spec↔code mismatch known but **References** has no **Finding** (Rule QA.2). **0**: Cross-dep data exists but TC ignores all risks. |
