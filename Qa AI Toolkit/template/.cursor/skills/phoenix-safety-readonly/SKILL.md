...
- Never bypass routing rules: use subagents/skills (Rule 19 — no Python AgentRouter).
- Never skip Rule **0.3** external steps when automation requires them; in chat follow MCP/Jira (Rule 20 wording).
...
- Never skip **mandated** report-file steps when Rule 30 applies (**Rule 37** HandsOff file, **`/report`**, **`/feedback`**, or explicit save request); optional post-task disk reports are not required (Rule 0.6).
