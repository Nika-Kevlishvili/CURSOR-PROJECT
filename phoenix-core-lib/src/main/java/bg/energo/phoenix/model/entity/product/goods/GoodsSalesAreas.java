...
@Table(name = "goods_sales_areas", schema = "goods")
public class GoodsSalesAreas extends BaseEntity {
    @Id
    @Column(name = "id")
    @SequenceGenerator(
            name = "goods_sales_areas_id_seq",
            sequenceName = "goods.goods_sales_areas_id_seq",
            allocationSize = 1
    )
    @GeneratedValue(
            strategy = GenerationType.SEQUENCE,
            generator = "goods_sales_areas_id_seq"
    )
    private Long id;
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "sales_areas_id", nullable = false)
