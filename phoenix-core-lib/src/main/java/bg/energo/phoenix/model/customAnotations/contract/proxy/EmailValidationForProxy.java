...
@Documented
@Constraint(validatedBy = {EmailValidationForProxy.EmailValidationForProxyImpl.class})
public @interface EmailValidationForProxy {
    ...
    class EmailValidationForProxyImpl implements ConstraintValidator<EmailValidationForProxy, ProxyAddRequest> {
        @Override
        public boolean isValid(ProxyAddRequest request, ConstraintValidatorContext context) {
            ...
            if(request.getProxyEmail() != null){
                if(!(request.getProxyEmail().length() > 1 && request.getProxyEmail().length() <= 512)){
                    isValid = false;
                    validationMessage.append("proxyEmail-[proxyEmail] email length should be between 1 and 512;");
                }
            }
            if(request.getAuthorizedProxyEmail() != null){
                if(!(request.getAuthorizedProxyEmail().length() > 1 && request.getAuthorizedProxyEmail().length() <= 512)){
                    isValid = false;
                    validationMessage.append("authorizedProxyEmail-[authorizedProxyEmail] email length should be between 1 and 512;");
                }
            }

            if(request.getProxyEmail()!=null && !patternMatches(request.getProxyEmail(),emailValidationRegex)){
                isValid = false;
                validationMessage.append("proxyEmail-[proxyEmail] email validation failed;");
            }
            if(request.getAuthorizedProxyEmail()!=null && !patternMatches(request.getAuthorizedProxyEmail(),emailValidationRegex)){
                isValid = false;
                validationMessage.append("authorizedProxyEmail-[authorizedProxyEmail] email validation failed;");
            }
            if(request.getAuthorizedProxyEmail() == null && request.getProxyEmail() == null){
                isValid = false;
                validationMessage.append("proxyEmail-[proxyEmail] shouldn't be empty;");
                validationMessage.append("authorizedProxyEmail-[authorizedProxyEmail] shouldn't be empty;");
            }
