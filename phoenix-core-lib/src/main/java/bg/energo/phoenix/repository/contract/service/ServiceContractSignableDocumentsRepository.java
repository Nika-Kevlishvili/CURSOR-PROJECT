...
@Repository
public interface ServiceContractSignableDocumentsRepository extends JpaRepository<ServiceContractSignableDocuments, Long> {
    ...
    @Query(nativeQuery = true, value = """
            with filtered_documents as (select csd.contract_detail_id             as cd_id,
                                        ...
                                        from service_contract.contract_signable_documents csd
            ...
            from service_contract.contracts c
                     join service_contract.contract_details cd on c.id = cd.contract_id
                     join filtered_documents fd on cd.id = fd.cd_id
                     join customer.customer_details cust_det on cd.customer_detail_id = cust_det.id
                     join service.service_details pd on cd.service_detail_id = pd.id
                     join service.service_templates pt on pd.id = pt.service_detail_id
                and pt.status = 'ACTIVE'
                and pt.service_template_type = 'EMAIL_TEMPLATE'
    ...
    @Modifying
    @Query("""
            update ServiceContractSignableDocuments d
    ...
    @Query("""
            ...
            where d.id in (select csd.documentId
                           ...
                           and csd.documentId = d.id)
            """)
    List<Document> getDocumentsForServiceContractByContractDetailId(Long detailId);
}
