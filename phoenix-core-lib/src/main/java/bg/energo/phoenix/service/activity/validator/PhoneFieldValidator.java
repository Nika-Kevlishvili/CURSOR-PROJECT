...
@Slf4j
public class PhoneFieldValidator implements SystemActivityFieldTypeValidator {
    ...
    private static void validate(String selectedValue, SystemActivityJsonField requestField, List<String> fieldValidationMessages, int index) {
        ...
        if (!selectedValue.matches(SubActivityRegExp.getPhonePattern())) {
            log.error("fields[%s].selectedValue-Provided field value is not a valid phone number;".formatted(index));
            fieldValidationMessages.add("fields[%s].selectedValue-Provided field value is not a valid phone number;".formatted(index));
        }
    }
