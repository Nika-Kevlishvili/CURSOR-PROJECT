...
@Slf4j
@Service
public class MassEmailCommunicationActivityService extends SystemActivityBaseService {
    ...
    public MassEmailCommunicationActivityService(ActivityRepository activityRepository,
    ...
    public Long create(CreateSystemActivityRequest request, SystemActivityConnectionType connectionType) {
        ...
        EmailCommunication emailCommunication = emailCommunicationRepository
                ...
                .orElseThrow(() -> new DomainEntityNotFoundException(
                                "objectId-mass email communication not found by ID %s and status in: %s".formatted(request.getObjectId(), List.of(EntityStatus.ACTIVE))
                        )
                );
    ...
    public SystemActivityResponse view(Long id) {
        ...
        EmailCommunicationActivity emailCommunicationActivity = emailCommunicationActivityRepository
                ...
                .orElseThrow(() -> new DomainEntityNotFoundException(
                        "systemActivityId-mass email communication activity not found by ID %s and status in: %s"
                                .formatted(response.getId(), List.of(EntityStatus.ACTIVE))
                ));

        EmailCommunication emailCommunication = emailCommunicationRepository
                .findById(emailCommunicationActivity.getEmailCommunicationId())
                .orElseThrow(() -> new DomainEntityNotFoundException("objectId-mass email communication not found by ID %s;".formatted(emailCommunicationActivity.getEmailCommunicationId())));
    ...
    public SystemActivityConnectionType getActivityConnectionType() {
        return SystemActivityConnectionType.MASS_EMAIL_COMMUNICATION;
    }

    @Override
    public List<SystemActivityShortResponse> getActivitiesByConnectedObjectId(Long id) {
        log.debug("Viewing activities for mass email communication, id {};", id);
