...
@Slf4j
@Service
public class AvgMonthlyEstConsUnderContractEvaluator extends PenaltyFormulaVariableEvaluationStrategy {

    @Override
    public PenaltyFormulaVariable getVariableType() {
        return PenaltyFormulaVariable.AVERAGE_MONTHLY_ESTIMATED_CONSUMPTION_UNDER_CONTRACT;
    }
    ...
    public Optional<BigDecimal> evaluate(PenaltyFormulaEvaluationArguments formulaEvaluationArguments,
        ...
        try {
            BigDecimal averageMonthlyEstimatedConsumptionUnderContract = contractVariables.getProductContractVariables()
                    .getEstimatedTotalConsumptionUnderContract()
                    .divide(BigDecimal.valueOf(12), getScale(), getRoundingMode());

            log.info("Calculated value for average monthly estimated consumption under contract: {}", averageMonthlyEstimatedConsumptionUnderContract);

            return Optional.of(averageMonthlyEstimatedConsumptionUnderContract);
        } catch (Exception e) {
            log.error("Unable to calculate average monthly estimated consumption under contract.", e);
            infoErrorMessages.add("%s-Unable to calculate average monthly estimated consumption under contract: exception during arithmetic operation."
                                          .formatted(INFORMATIONAL_ERROR_MESSAGE_INDICATOR));
            return Optional.empty();
        }
    }
