...
@Service
@Slf4j
public class AvgMonthlyActualConsUnderContractEvaluator extends PenaltyFormulaVariableEvaluationStrategy {
    @Override
    public PenaltyFormulaVariable getVariableType() {
        return PenaltyFormulaVariable.AVERAGE_MONTHLY_ACTUAL_CONSUMPTION_UNDER_CONTRACT;
    }
    ...
    public Optional<BigDecimal> evaluate(PenaltyFormulaEvaluationArguments formulaEvaluationArguments, ContractType contractType, PenaltyCalculationContractVariables contractVariables, List<String> infoErrorMessages) {
        if (Objects.isNull(contractVariables.getProductContractVariables().getAvgMonthlyActualConsumptionUnderContract())) {
            log.error("Unable to calculate average monthly actual consumption under contract.");
            infoErrorMessages.add("%s-Unable to calculate average monthly actual consumption under contract."
                    .formatted(INFORMATIONAL_ERROR_MESSAGE_INDICATOR));
            return Optional.empty();
        }

        log.info("Calculated value for average monthly actual consumption under contract: {}",
                contractVariables.getProductContractVariables().getAvgMonthlyActualConsumptionUnderContract());

        return Optional.of(contractVariables.getProductContractVariables().getAvgMonthlyActualConsumptionUnderContract());
    }
}
