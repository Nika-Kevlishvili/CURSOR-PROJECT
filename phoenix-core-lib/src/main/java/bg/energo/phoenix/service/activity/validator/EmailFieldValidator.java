...
@Slf4j
public class EmailFieldValidator implements SystemActivityFieldTypeValidator {
    ...
    private static void validate(String selectedValue, SystemActivityJsonField requestField, List<String> fieldValidationMessages, int index) {
        ...
        if (!selectedValue.matches(SubActivityRegExp.getEmailPattern())) {
            log.error("fields[%s].selectedValue-Provided field value is not a valid email;".formatted(index));
            fieldValidationMessages.add("fields[%s].selectedValue-Provided field value is not a valid email;".formatted(index));
        }
    }
