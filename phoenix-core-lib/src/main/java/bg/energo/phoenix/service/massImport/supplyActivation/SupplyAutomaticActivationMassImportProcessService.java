...
import bg.energo.phoenix.model.request.contract.pod.MassImportActivationRequest;
...
@Slf4j
@Service
public class SupplyAutomaticActivationMassImportProcessService extends AbstractMassImportProcessService {
    private final ContractPodService contractPodService;
    @Value("${app.cfg.supplyAutomaticActivation.massImport.numberOfRowsPerTask}")
    private Integer numberOfRowsPerTsk;
    @Value("${app.cfg.supplyAutomaticActivation.massImport.numberOfTasksPerThread}")
    private Integer numberOfCallablesPerThread;
    @Value("${app.cfg.supplyAutomaticActivation.massImport.numberOfThreads}")
    private Integer numberOfThreads;

    public SupplyAutomaticActivationMassImportProcessService(ContractPodService contractPodService,
    ...
    protected String processRow(Row row, Set<String> permissions, String processSysUserId, LocalDate date, Long processRecordInfo) {
        ...
        if (row.getCell(0) != null || row.getCell(0).getCellType() != CellType.BLANK) {
            ...
            this.contractPodService.startMIActivation(
                    new MassImportActivationRequest(
    ...
    public boolean supports(EventType eventType) {
        return EventType.SUPPLY_AUTOMATIC_ACTIVATION_MASS_IMPORT_PROCESS.equals(eventType);
    }
