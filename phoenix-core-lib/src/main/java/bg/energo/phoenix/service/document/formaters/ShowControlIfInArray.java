...
public class ShowControlIfInArray implements DocumentFactoryBuilder.Handler {
        
    @Override
    public Handled handle(Object value, String metadata, String path, int position, hr.ngs.templater.Templater templater) {
        if (metadata.startsWith("showControlIfInArray(")) {
            //Extract the matching expression
            String expression = metadata.substring("showControlIfInArray(".length(), metadata.length() - 1);
            try {
