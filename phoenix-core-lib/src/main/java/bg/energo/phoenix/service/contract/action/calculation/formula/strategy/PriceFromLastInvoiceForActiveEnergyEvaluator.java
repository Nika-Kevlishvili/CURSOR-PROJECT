...
@Slf4j
@Service
public class PriceFromLastInvoiceForActiveEnergyEvaluator extends PenaltyFormulaVariableEvaluationStrategy {
    @Override
    public PenaltyFormulaVariable getVariableType() {
        return PenaltyFormulaVariable.PRICE_FROM_LAST_INVOICE_FOR_ACTIVE_ENERGY;
    }
    ...
    public Optional<BigDecimal> evaluate(PenaltyFormulaEvaluationArguments formulaEvaluationArguments, ContractType contractType, PenaltyCalculationContractVariables contractVariables, List<String> infoErrorMessages) {
        if (Objects.isNull(contractVariables.getProductContractVariables().getPriceFromLastInvoiceForActiveEnergy())) {
            log.error("Unable to calculate price from last invoice for active energy.");
            infoErrorMessages.add("%s-Unable to calculate price from last invoice for active energy."
                    .formatted(INFORMATIONAL_ERROR_MESSAGE_INDICATOR));
            return Optional.empty();
        }

        log.info("Calculated value for price from last invoice for active energy: {}",
                contractVariables.getProductContractVariables().getPriceFromLastInvoiceForActiveEnergy());

        return Optional.of(contractVariables.getProductContractVariables().getPriceFromLastInvoiceForActiveEnergy());
    }
}
