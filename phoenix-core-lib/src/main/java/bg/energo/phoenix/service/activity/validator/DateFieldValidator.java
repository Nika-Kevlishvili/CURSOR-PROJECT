...
@Slf4j
public class DateFieldValidator implements SystemActivityFieldTypeValidator {
    ...
    private static void validate(String selectedValue, List<String> fieldValidationMessages, int index) {
        if (!EPBDateUtils.isValidLocalDate(selectedValue, DateTimeFormatter.ISO_DATE)) {
            log.error("fields[%s].selectedValue-Provided field value is not a valid date;".formatted(index));
            fieldValidationMessages.add("fields[%s].selectedValue-Provided field value is not a valid date;".formatted(index));
        }
    }
