...
@Slf4j
@Service
public class ActualPavEvaluator extends PenaltyFormulaVariableEvaluationStrategy {
    @Override
    public PenaltyFormulaVariable getVariableType() {
        return PenaltyFormulaVariable.ACTUAL_PAV;
    }
    ...
    public Optional<BigDecimal> evaluate(PenaltyFormulaEvaluationArguments formulaEvaluationArguments, ContractType contractType, PenaltyCalculationContractVariables contractVariables, List<String> infoErrorMessages) {
        if (Objects.isNull(contractVariables.getProductContractVariables().getActualPav())) {
            log.error("Unable to calculate actual Pav.");
            infoErrorMessages.add("%s-Unable to calculate actual Pav."
                    .formatted(INFORMATIONAL_ERROR_MESSAGE_INDICATOR));
            return Optional.empty();
        }

        log.info("Calculated value for actual Pav: {}",
                contractVariables.getProductContractVariables().getActualPav());

        return Optional.of(contractVariables.getProductContractVariables().getActualPav());
    }
}
