...
@Service
@Slf4j
public class AvgDailyActualConsUnderContractForPodEvaluator extends PenaltyFormulaVariableEvaluationStrategy {
    @Override
    public PenaltyFormulaVariable getVariableType() {
        return PenaltyFormulaVariable.AVERAGE_DAILY_ACTUAL_CONSUMPTION_UNDER_CONTRACT_FOR_POD;
    }
    ...
    public Optional<BigDecimal> evaluate(PenaltyFormulaEvaluationArguments formulaEvaluationArguments, ContractType contractType, PenaltyCalculationContractVariables contractVariables, List<String> infoErrorMessages) {
        if (Objects.isNull(contractVariables.getProductContractVariables().getAvgDailyActualConsumptionUnderContractForPod())) {
            log.error("Unable to calculate average daily actual consumption under contract for pod.");
            infoErrorMessages.add("%s-Unable to calculate average daily actual consumption under contract for pod."
                    .formatted(INFORMATIONAL_ERROR_MESSAGE_INDICATOR));
            return Optional.empty();
        }

        log.info("Calculated value for average daily actual consumption under contract for pod: {}",
                contractVariables.getProductContractVariables().getAvgDailyActualConsumptionUnderContractForPod());

        return Optional.of(contractVariables.getProductContractVariables().getAvgDailyActualConsumptionUnderContractForPod());
    }
}
