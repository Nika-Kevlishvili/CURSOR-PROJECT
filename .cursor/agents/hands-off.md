...
9. **Agent questions after report** – After the report is sent, collect **questions from each participating agent** (as needed), including **PlaywrightTestValidatorAgent**; each question MUST be **attributed** to the agent (e.g. `[AgentName]: <question>`). Send this list of questions **after** the report to the same Slack recipients (tester + AI report channel). See `.cursor/rules/handsoff_playwright_report.mdc` §7.
...
## Mandatory Slack Compact Format

When sending test results to Slack as a compact report (including follow-up compact updates), use this format **always**:

1. Header line: `<JIRA_KEY> compact passed report` (or failed report when applicable).
2. One run-summary line: `Run summary: <passed> passed, <failed> failed, <skipped> skipped`.
3. Numbered test case list.
4. Test case name in **bold**.
5. First row per case must contain status details:
   - `N. **<TC-NAME>** — status code: <code or range> — status: <Passed|Failed|Skipped>`
6. Second row per case must contain a short human-readable description of what is validated.
7. Third row per case must contain the request details used for API checks:
   - method + endpoint + key params (query/path/body as relevant), e.g.
   - `Request: GET /path?param1=...&param2=...`
8. Keep wording concise and consistent across all cases.

Important:
- This compact format is **in addition to** the full report requirement in Step 8 (do not replace full report delivery unless user explicitly asks for compact-only).
- If a test is skipped due to missing configuration, include the missing parameter name explicitly in the test case row/description.
