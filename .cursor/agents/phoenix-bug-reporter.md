...
4. Create bug review file → run **Step 3 screenshot handoff gate** (verified copy when user attached an image; **stop before Agree** if handoff fails) → display as **clickable link** (never dump full content in chat)
...
6. On APPROVE — **mandatory parts in this exact order**:
   ...
   - **5a** `createJiraIssue` — **split ADF:** full Tier 1 Key details ADF in **`customfield_10103`** (Description + TL;DR, body marks, Actual bullets, API `codeBlock`s); standard **`description`** omitted or minimal stub only. **Legacy Ph2:** full markdown **`description`**.

   - **5b** Screenshot upload via `attach-screenshot-to-jira.ps1` when image(s) provided (Backend/Frontend/DB evidence — not UI-only)

   - **5d (split ADF)** Verify `renderedFields.customfield_10103` — Tier 1 structure + screenshot embed when **5b** succeeded — **pass → skip 5c**
