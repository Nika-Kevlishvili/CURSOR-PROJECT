...
- **Elasticsearch log analysis:** search application logs via **ElasticsearchDev** (Dev/Dev2), **ElasticsearchExperiment** (Experiments — same Dev cluster URL, keyword filters), **ElasticsearchTest** (Test/PreProd), **ElasticsearchTest2ES** / **ElasticsearchTest2SLR** (Test 2 ES / SLR — same Test cluster URL, keyword filters), or **ElasticsearchProd** — see Step 4c in SKILL. Include **`### Elasticsearch Log Analysis`** section in every completed validation.
