...
**Path 2 of 2** — see **`Cursor-Project/config/template/Slack_reporting_paths.md`**. Path 1 = bug validation (unchanged).

User-triggered Slack delivery of **scoped** Playwright results.
...
- **Rule DPR.0** (`.cursor/rules/workflows/playwright_detailed_reporting.mdc`): for **path 2**, **`playwright-report-detailed.md`** is **generated and uploaded** together with the smart **ScopedPlaywright** `.md` when **`playwright-report.json`** exists. See **`Cursor-Project/config/playwright/README-detailed-reporting.md`** for the generator.
...
9. **Recipients** (when Jira is known):
