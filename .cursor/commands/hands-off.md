...
2. Finder MUST follow **Rule 35a**: **Jira MCP + codebase + deep Confluence exploration** — no local merge-history archaeology/git-snapshot; **technical_details** from ticket + code (GitLab MR only if user explicitly asked).
...
- **Rule 35/35a:** Cross-dependency-finder must run before test case generator (Jira + codebase + deep Confluence exploration; no local merge-history archaeology/git-snapshot); **technical_details** when a Jira key is provided.
