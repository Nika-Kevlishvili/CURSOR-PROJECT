...
try {
    $SourceBranch = "Backend-staging"
    
    Write-Host "=== Syncing cursor branch with Backend-staging ===" -ForegroundColor Cyan
    Write-Host ""
    
    # Step 1: Check current branch
    $currentBranch = git branch --show-current
    Write-Host "Current branch: $currentBranch" -ForegroundColor Yellow
    Write-Host "Source branch: Backend-staging" -ForegroundColor Yellow
    Write-Host ""
    ...
    # Step 3: Fetch latest from Backend-staging
    Write-Host ""
    Write-Host "Fetching latest changes from Backend-staging branch..." -ForegroundColor Cyan
    git fetch origin Backend-staging
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to fetch from origin/Backend-staging"
        if ($stashed) {
    ...
    # Step 4: Check if cursor is behind Backend-staging
    $cursorCommit = git rev-parse cursor
    $stagingCommit = git rev-parse origin/Backend-staging
    
    if ($cursorCommit -eq $stagingCommit) {
        Write-Host ""
        Write-Host "Cursor branch is already up to date with Backend-staging." -ForegroundColor Green
    } else {
        # Step 5: Merge Backend-staging into cursor
        Write-Host ""
        Write-Host "Merging origin/Backend-staging into cursor branch..." -ForegroundColor Cyan
        
        # Try merge
        git merge --no-edit origin/Backend-staging 2>&1 | Tee-Object -Variable mergeOutput
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($conflictFiles) {
                ...
                foreach ($conflict in $allConflicts) {
                    ...
                    elseif ($status -eq "UD") {
                        # Deleted by them (Backend-staging) - keep file (cursor version)
                ...
                git commit -m "Merge Backend-staging into cursor - keeping cursor branch changes" --no-edit
    ...
    Write-Host ""
    Write-Host "Cursor branch has been synced with Backend-staging." -ForegroundColor Green
