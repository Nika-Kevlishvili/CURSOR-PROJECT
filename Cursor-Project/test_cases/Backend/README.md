...
| `Put_Update_Existing_Contract_PHN2130.md` | Sales Portal PUT /sales-portal/contract — private / private-with-business customers (legal entity TC-BE-22 only); field omission semantics, direct debit, proxy/manager, customer swap; different product on contract allowed; product swap version mismatch; third-tab productParameters; price-component restriction; fixed IAP accepted; add second POD; customer local/foreign address; contract status transitions (ENTERED_INTO_FORCE, ACTIVE_IN_TERM, TERMINATED); KYC; validation errors | PHN-2130 | TC-BE-1 through TC-BE-34 |
| `Request_for_disconnection_supplier_type_filtering_PDT_2957.md` | Request for Disconnection — POD list filtered by `supplierType` (CURRENT active contract-POD / PREVIOUS inactive); multi-customer shared POD; allSelected create; EXECUTED regression untouched; validation | PDT-2957 | TC-BE-1 through TC-BE-14 |
| `Invoice_correction_wrong_scale_inclusion_PDT_3014.md` | Invoice correction (BY_SCALES, volume-only) — wrong POD / zero-volume / wrong-period scale must not enter correction; reproduces prod run 3590 pattern (296173 decoy) | PDT-3014 | TC-BE-1 through TC-BE-4 |
