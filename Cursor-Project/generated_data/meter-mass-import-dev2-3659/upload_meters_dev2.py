#!/usr/bin/env python3
"""Upload meter mass-import xlsx files one by one to Dev2.

Waits until each PROCESS_METER_MASS_IMPORT reaches COMPLETED or CANCELED
...
SRC_DIR = Path("/Users/lukachrikishvili/Desktop/mass imports/500k meters")
PROGRESS_PATH = SRC_DIR / "upload_progress.json"
BASE = "https://devapps.energo-pro.bg/backend/phoenix2-dev"
UPLOAD_URL = f"{BASE}/mass-import/METERS/files/upload"
...
def list_meter_processes(token: str, size: int = 30) -> list[dict]:
    qs = urllib.parse.urlencode(
        {
            ...
            "sortBy": "CREATE_DATE",
            "sortDirection": "DESC",
            "prompt": "PROCESS_METER_MASS_IMPORT",
            "searchBy": "NAME",
        }
    )
...
def resolve_file_count() -> int:
    files = sorted(SRC_DIR.glob("meter-mass-import-dev2-3659-part-*-of-*.xlsx"))
    if not files:
        raise SystemExit(f"NO_FILES in {SRC_DIR}")
    return len(files)
...
def main() -> None:
    ...
    file_count = resolve_file_count()
    results = []
    known_ids = {
        item["id"]
        for item in list_meter_processes(token, size=80)
        if str(item.get("name", "")).startswith("PROCESS_METER_MASS_IMPORT_")
    }
    print(f"KNOWN_METER_PROCESSES {len(known_ids)} START_PART={START_PART} FILE_COUNT={file_count}", flush=True)
    ...
    for part in range(START_PART, file_count + 1):
        path = SRC_DIR / f"meter-mass-import-dev2-3659-part-{part:03d}-of-{file_count:03d}.xlsx"
        if not path.exists():
        ...
        for _ in range(15):
            time.sleep(2)
            for item in list_meter_processes(token, size=30):
                name = str(item.get("name", ""))
                pid = item.get("id")
                if name.startswith("PROCESS_METER_MASS_IMPORT_") and pid not in known_ids:
                    process_id = pid
                    break
            if process_id:
                break
        ...
        row = {
            ...
            "processName": f"PROCESS_METER_MASS_IMPORT_{process_id}",
        }
        results.append(row)
        save_progress({"completed": results, "nextPart": part + 1})
        print(f"FINISHED {path.name} process={process_id} status={last_status}", flush=True)
