...
Uploads a **local file** (e.g. a scoped Playwright `ScopedPlaywright_*.md` report) to a Slack **channel** or **DM** using Slack Web API external upload (`files.getUploadURLExternal` → binary POST → `files.completeUploadExternal`).
...
**Scoped Slack path 2 — mandatory files:** Deliver **two** attachments when both exist: (1) smart report **`ScopedPlaywright_*.md`**, (2) **`Cursor-Project/EnergoTS/playwright-report-detailed.md`** from **`generate-detailed-report.mjs`** (run from **`EnergoTS/`**; same folder as **`playwright-report.json`**). Plus the short three-block Slack message — never replace files with full narrative in chat. Run uploads **after** the short Slack text. For **each** file, upload to (1) Tester **user ID** (DM) if present, then (2) **#ai-report** **`C0AK96S1D7X`** (up to **four** uploads per file pair). Use **`_run-upload-with-dotenv.ps1`** if the token lives in **`EnergoTS/.env`** as **`SLACK_API_TOKEN`**.
...
powershell -ExecutionPolicy Bypass -File "Cursor-Project/config/slack/upload-file-to-slack.ps1" `
  -FilePath "Cursor-Project/reports/Chat reports (ScopedPlaywright)/2026/may/01/PHN-2823.md" `
  -ChannelOrUserId "C0AK96S1D7X" `
  -InitialComment "Detailed Playwright report (scoped run)"
```
