...
## Report Attachment (mandatory — manual verification links)

**Every** `test()` in `tests/cursor/` must end with portal links for manual tester verification.

Import the shared helper (path relative to your spec under `tests/cursor/`):

```typescript
import { attachManualVerificationLinks } from './shared/manual-verification-links.fixtures';
```

Last step in each test:
...
Produces: console plain-text block, Playwright HTML attachments (plain + JSON + entity snapshot), and **Portal data links** in `generate-detailed-report.mjs` output.

`SAVE_OBJECT_LINKS=true` is optional extra logging inside `ResponseLinker` only — attachments work **without** it.
