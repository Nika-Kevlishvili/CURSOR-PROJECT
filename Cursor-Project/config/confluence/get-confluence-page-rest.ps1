<#
.SYNOPSIS
    Fetches a single Confluence Cloud wiki page via REST (read-only) using Basic auth.

.DESCRIPTION
    Intended as the operational helper for Confluence REST fallback when MCP is unavailable.
    Uses CONFLUENCE_WIKI_BASE or CONFLUENCE_URL, plus JIRA_EMAIL/JIRA_API_TOKEN or
    CONFLUENCE_EMAIL/CONFLUENCE_API_TOKEN. Never log secrets.

.PARAMETER PageId
    Numeric Confluence page ID (from URL .../pages/<id>/...).

.PARAMETER Api
    v1 = rest/api/content with expand (default). v2 = wiki/api/v2/pages.

.PARAMETER OutFile
    Optional path to write JSON instead of printing to stdout.

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File get-confluence-page-rest.ps1 -PageId '779517953'
#>
param(
